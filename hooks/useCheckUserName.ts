...
function useCheckUserName({
  ...
  async function queryNameCharacter() {
    ...
    // Ensure nameSearch is a string and convert it to lowercase
    const searchStrings = Array.isArray(nameSearch) ? nameSearch.filter(Boolean) : [nameSearch].filter(Boolean);
    const lowercaseSearchStrings = searchStrings.map((searchString) => searchString?.toLowerCase()).filter(Boolean);

    // Create a query to find usernames that start with lowercase 'n' or uppercase 'N'
    const q = query(
      Ref,
      where('usernameQuery', 'array-contains-any', lowercaseSearchStrings)
    );
