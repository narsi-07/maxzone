...
const App: React.FC = () => {
   ...
   const renderCalendar = () => {
    ...
    if (isWeekView) {
        // Weekly view: Set the start and end dates based on the selected week
    ...
    const renderWeekView = () => {
        ...
        // Calculate the start of the week to highlight
        const weekStartDate = new Date(startDate);
        weekStartDate.setDate(startDate.getDate() - startDate.getDay()); // Start of the week (Sunday)
    
        // Generate a grid of 30 days for the month
        for (let i = 0; i < 30; i++) {
            const date = new Date(today.getFullYear(), today.getMonth(), today.getDate() - today.getDate() + i + 1);
            const dateCell = createDateCell(date, false);
    
            // Highlight the current week range
            if (date >= weekStartDate && date < new Date(weekStartDate.getFullYear(), weekStartDate.getMonth(), weekStartDate.getDate() + 7)) {
